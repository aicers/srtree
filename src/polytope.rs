...
impl<const dimension: usize, T> Polytope<T, dimension> where T: Copy + Add<Output = T> + AddAssign + Sub<Output = T> + SubAssign + Mul<Output = T> + MulAssign + Ord + Default {
    
    pub fn from_points(points: Vec<[T; dimension]>) -> Polytope<T, dimension> {
        Polytope { rect: Rect::from_points(&points), sphere: Sphere::from_points(&points) }
    }
